...
float RollBack_measured = 0.0; //2^10 bits
...
void loop() {
  ...
  if (millis() - time_old >= 10){
      ...
      Roll_outcmd(AgriQJoyRx.VALUE_J1leftright); // Open loop incremental control
      ...
      MFL_outcmd(PID_MFL.ctrl -100); //Experimental offset
      ...
      PID_MFR.ctrl = mapfloat(PID_MFR.ctrl, -torque_max, torque_max, 4095, 0); //right motor: 0 fwd  4095 bwd  no current 2000
      MFR_outcmd(PID_MFR.ctrl -120); //Experimental offset
      ...
      if (AgriQJoyRx.VALUE_SW2 > 1600 && lambda > 0.1){ // IF AWD is ENABLED and AgriQ moving forward
