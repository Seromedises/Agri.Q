...
void MFL_Odometry(){
  MFL.odometry = MFL.odometry + r_wheel * MFL.omega_measured * time_sample / MFL.tau;
}
void MFR_Odometry(){
  MFR.odometry = MFR.odometry + r_wheel * MFR.omega_measured * time_sample / MFR.tau;
}
void Advance_ResetCL(){
  Advance_reference = 0.5*(MFL.odometry + MFR.odometry);
  AgriQAutoAdvance = 0;
}

float AgriQAutoAdvanceFwd(float Advance_reference, float Odometry_measured, bool AgriQAutoAdvance){

  float lambda_advance = 0.0;
  float lambda_advance_max = 0.25;

  if (AgriQAutoAdvance == 1 && EnableFrontMotors == 1){ 
    if(Advance_reference - Odometry_measured >= 0.01){
     // ADVANCE
     lambda_advance = 2.5 * (Advance_reference - Odometry_measured) + 0.15;
     lambda_advance = constrain(lambda_advance, 0.0, lambda_advance_max);
    }
    else {
      AgriQAutoAdvance = 0;
      // STOP
